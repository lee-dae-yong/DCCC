				...
				if(property.getNodeType() == Node.ELEMENT_NODE) {
					...
					for(Method method : methods){
						if(method.getName().equals(setMethodName)) {
							//System.out.printf("className : %s, name : %s, setMethod : %s\n",className,name,setMethodName);
							
							method.invoke(applicationContext.get(eleBean.getAttribute("id")), applicationContext.get(ref));
							//System.out.println("실행할 인스턴스 : "+applicationContext.get(eleBean.getAttribute("id")));
							//System.out.println("파라미터 : "+applicationContext.get(ref));
						}
					}
				}
			}
